...
// Explicitly expose the X-CSRF-Token header so that browsers can read it on
// cross‑origin requests.  Without this, the token header would not be
// available to client JavaScript when requests include credentials.
...
// Always send a CSRF cookie/header to the frontend.  csrf_token() will
// normalise the session keys so that both `csrf` and `csrf_token` refer to
// the same random value (see api/csrf.php).  This avoids mismatches
// between multiple token names used across the codebase.
send_csrf_cookie();
...
function get_csrf_token() {
    // Return the unified CSRF token generated by csrf.php.  The
    // csrf_token() helper normalises both `csrf` and `csrf_token` keys in
    // the session, so this function simply returns that value.  This ensures
    // that the admin endpoint /admin/csrf returns the same token that is
    // sent in the XSRF‑TOKEN cookie and X‑CSRF‑Token header by send_csrf_cookie().
    return csrf_token();
}
...
function verify_csrf_token() {
    // This function is retained for backwards compatibility but now delegates
    // to verify_csrf_or_fail() from api/csrf.php.  The original
    // implementation validated against $_SESSION['csrf_token'], but the
    // application previously generated a separate $_SESSION['csrf'] value
    // when sending cookies.  Delegating to verify_csrf_or_fail() ensures
    // that both keys are normalised and checked consistently.
    verify_csrf_or_fail();
}
...
// Cho phép override phương thức khi host chặn PUT/DELETE
if ($method === 'POST') {
    $override = $_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE'] ?? null;
    if ($override) $method = strtoupper($override);
    else {
        $tmp = json_decode(file_get_contents('php://input'), true) ?: [];
        if (!empty($tmp['_method'])) $method = strtoupper($tmp['_method']);
    }
}

require_once __DIR__ . '/toc_utils.php';

// Enforce CSRF token for admin routes on state‑changing methods.  We use
// verify_csrf_or_fail() from csrf.php directly to validate the
// X‑CSRF‑Token header against the unified session token.  This avoids
// mismatches between different token names (e.g. `csrf` vs `csrf_token`)
// and ensures consistent CSRF protection across all admin endpoints.
...
try {
    ...
    if (preg_match('#^/admin/books/(?P<id>\d+)$#', $path, $m) && $method === 'PUT') {
        $admin = require_admin();
        // CSRF verification is performed globally for admin routes.  Avoid
        // calling verify_csrf_or_fail() a second time here, which could
        // incorrectly validate against an outdated token and result in a
        // spurious 403 error when updating a book.
        $bookId = (int)$m['id'];
        $input = get_json_input();
        $title = isset($input['title']) ? trim($input['title']) : null;
